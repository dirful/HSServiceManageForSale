...
public class QuotedPriceService extends SearchService implements
	 ...
	 */
	@Override
	public List<Process> getQuotedPriceProcessByID(String condition) {
		return processDao.getByCondition(condition);
	}
	...
	@Override
	public List<Map<String, Object>> getQuotedPriceByConditon(String condition) {
		if (condition != null) {
			condition += " AND ";
		}
		condition += " e.ID = qp.OfferorsID AND  qp.processID=pd.ProcessID AND pd.submitAccountID = qp.ID  ORDER BY qp.joinTime DESC ";
		...
		String[] properties = new String[] {
				...
				"CASE pd.state WHEN '1' THEN '待审核'WHEN '2' THEN '已审核'ELSE '数据库错误' END as MYSTATE",
...
//				,"pd.state AS MYSTATE" 
		...
		String baseEntity = " quotedprice AS qp , employee AS e , processdescription AS pd";
		String joinEntity = "";
		List<Map<String, Object>> list = this.originalSearchForeign(properties,
				baseEntity, joinEntity, null, condition, false);
	...
	public List<Map<String, Object>> getQuotedPriceByConditonWithpaging(
		...
		condition += " e.ID = qp.OfferorsID AND  qp.processID=pd.ProcessID AND pd.submitAccountID = qp.ID";
...
//						project AS pj,
	 ...
	 */
	@Override
	public List<Map<String, Object>> getMySaleLogByConditonWithpaging(
			String condition, int row, int page) {
		if (condition != null) {
			condition += " AND ";
		}
		condition += " e.ID = sl.OfferorsID AND  sl.processID=pd.ProcessID AND pd.submitAccountID = sl.ID";
/*		condition += " e.ID = qp.OfferorsID AND pj.ID = qp.ProjectID AND qp.processID=pd.ProcessID ";*/
...
//								,"pd.state AS MYSTATE" 
						...
						"employee AS e,processdescription AS pd",
//						project AS pj,
						null, condition, false, null, "sl.CreateTime", " DESC ",
						row, page);
	}
	@Override
	public List<Map<String, Object>> getMySaleLogByConditon(String condition){
		if (condition != null) {
			condition += " AND ";
		}
		condition += " e.ID = sl.OfferorsID AND  sl.processID=pd.ProcessID AND pd.submitAccountID = sl.ID";
...
//				,"pd.state AS MYSTATE" 
		...
		String baseEntity = "salelog AS sl,employee AS e,processdescription AS pd";
		String joinEntity = "";
		List<Map<String, Object>> list = this.originalSearchForeign(properties,
				baseEntity, joinEntity, null, condition, false);
