...
$(function(){
    $("#dg").datagrid({
       ...
       striped:true,
       singleSelect:true,
       url:'quotedPriceController/getBackEmployeeInfo.do',
       loadMsg:"数据装载中......",
       columns:[[
          {field:'ID',title:'ID',hidden:true,width:50},
          {field:'quotedPriceCode',title:'编码',width:100},
          {field:'employeeName',title:'用户名称',width:100},
   ...
   function sear(n){
	   ...
	   case 2:find ="DATE_FORMAT(quotedprice.joinTime,'%Y-%m-%d') BETWEEN '"+$.trim($('#timeBefore').val())+"' and '"+$.trim($('#timeAfter').val())+"'";
	   }
	   if(find){
		   $('#dg').datagrid({
...
//		  if(r){
//			  $.post("quotedPriceController/deleteBackEmployeeInfo.do",{IDs:ids},function(data){
//				  $('#dg').datagrid('reload');
//			  });
//		  } else{
//			  alert("删除失败");
